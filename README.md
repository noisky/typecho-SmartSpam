...
IP 屏蔽规则每行填写一项。普通地址仅匹配完整 IP，例如 `192.168.1.10`；需要匹配网段时可使用 `*`，例如 `192.168.*.*`。空行会被忽略。
...
插件只接受 `none`、`waiting`、`spam` 和 `abandon` 四种动作值；缺失或异常的配置会回退到安全默认值，避免中断评论处理。昵称和评论长度配置缺失或格式错误时，分别使用 `1～15` 和 `5～200`。
...
协议只改变错误展示方式，不改变过滤结果。支持 `prg-v1` 的 Geetest 等评论插件可以与 SmartSpam 共用同一套主题错误回显逻辑。

## 兼容性

- 未声明 `prg-v1` 的主题继续显示原有 500 页面；
- `waiting` 和 `spam` 仍按原逻辑保存评论，不触发失败提示；
- SmartSpam 与支持 `prg-v1` 的其他评论插件共用协议字段和 `__typecho_comment_error` Cookie，但各插件仍独立执行校验；
- Typecho 核心校验或其他未接入协议的插件异常不在本协议范围内。

过滤器会兼容其他插件返回空值或缺少字段的情况，并将评论字段规范为预期类型。关键词列表中的空行会被忽略；UTF-8 字符长度优先使用 `mb_strlen()`，未安装 `mbstring` 时自动使用正则方式计算。

## 版本记录

- 3.0.1（2026-09-20）：增强评论数据与缺失配置的容错处理，限制过滤动作取值，修正 IP 精确匹配与通配段匹配，并改进 UTF-8 字符长度计算；
